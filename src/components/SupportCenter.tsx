...
export default function SupportCenter({ faqs }: { faqs: Faq[] }) {
  ...
  function addAttachments(files: FileList | null) {
    ...
    const images = Array.from(files)
      .filter((file) => file.type.startsWith("image/") && file.size <= MAX_SUPPORT_ATTACHMENT_BYTES)
      .slice(0, Math.max(0, MAX_SUPPORT_ATTACHMENTS - attachments.length))
