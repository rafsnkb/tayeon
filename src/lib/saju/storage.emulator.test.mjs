...
suite("보관 만료 시각을 만들 때 박는다 — 코드용 ISO 와 TTL용 Timestamp 가 같은 시각이다", async () => {
  const uid = newUid();
  const reading = await openedReading(uid);
  const saved = await storage.getSajuReading(uid, reading.id);

  // 기간 상수를 고쳐도 이미 팔린 리포트가 안 움직이는 건 "만들 때 박는다"에서 나온다.
  const days = (Date.parse(saved.expiresAt) - Date.parse(saved.createdAt)) / 86400000;
  assert.equal(Math.round(days), 30);

  // TTL 은 Timestamp 필드만 인식한다 — 문자열만 두면 정책을 켜도 아무것도 안 지워진다.
  assert.equal(typeof saved.expiresAtTs.toDate, "function", "expiresAtTs 가 Timestamp 로 복원되지 않았다");
  assert.equal(saved.expiresAtTs.toDate().toISOString(), saved.expiresAt, "두 필드의 시각이 어긋났다");

  // 갓 만든 리포트는 만료가 아니고 게이트도 열려 있다.
  assert.equal(storage.isSajuReadingExpired(saved), false);
  assert.deepEqual(storage.pageGateReason(saved, [], 1), { ok: true });
});
