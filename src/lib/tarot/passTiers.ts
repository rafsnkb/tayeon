...
/** 이용권 카드 한 장에 필요한 색 묶음 — 테두리 `border` 와 배지 알약의 `tagBg`/`tagText`.
 *
 *  배경 텍스처(`bg`)도 여기 있었다. 등급별 성운 4장(`/textures/tier-1~4.jpg`)을 12종이 나눠
 *  쓰는 구조였는데, 2026-09-25 에 상품마다 그림이 생기면서 안 쓰게 돼 파일까지 지웠다
 *  (사용자 요청). 배경은 `timePassArt()` 에서 가져온다. */
export type PassTier = { border: string; tagBg: string; tagText: string };
...
const TIER = {
  teal: { border: "#9de9ed", tagBg: "rgba(12, 68, 86, 0.86)", tagText: "#9de9ed" },
  blue: { border: "#2f8bee", tagBg: "rgba(15, 52, 98, 0.86)", tagText: "#66b0ff" },
  violet: { border: "#8335d6", tagBg: "rgba(56, 24, 82, 0.86)", tagText: "#a04ff8" },
  ...
  coral: { border: "var(--point)", tagBg: "rgba(82, 17, 59, 0.86)", tagText: "var(--point)" },
