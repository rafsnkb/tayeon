/** 카드 그림(`asset/resource/*.png`)을 마지막으로 교체한 날. **그림을 갈아끼우면 반드시 같이
 *  올릴 것** — 안 올리면 사용자 화면에 신구 그림이 섞여 나온다(2026-09-26 사용자 리포트).
 *
 *  이미지 라우트가 `Cache-Control: immutable, max-age=1년` 을 단다. URL 이 그대로면 브라우저는
 *  **재검증조차 하지 않고** 1 년간 옛 그림을 쓴다. 이미 캐시된 사람은 서버에서 헤더를 바꿔도
 *  구제되지 않으므로, 캐시를 깨려면 URL 자체가 달라져야 한다 — 그래서 이 값을 쿼리로 붙인다.
 *
 *  잊고 안 올리는 걸 막으려고 테스트가 이 값과 실제 파일 수정일을 대조한다
 *  (`cards.test.mjs`). 그림만 바꾸고 이 줄을 안 고치면 테스트가 깨진다. */
export const CARD_ART_VERSION = "2026-09-25";
