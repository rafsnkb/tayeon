...
import { USERS, BIRTHDAY_COUPON_GRANTS, PENDING_REWARDS } from "@/lib/firestore/collections";
...
export async function grantBirthdayCoupon(uid: string, birthdayKey: string, issuedAt = new Date().toISOString()) {
  ...
  const ledgerRef = userRef.collection(BIRTHDAY_COUPON_GRANTS).doc(birthdayKey);
  const rewardRef = userRef.collection(PENDING_REWARDS).doc();
