          ...
          return (
            ...
            >
              ...
              {/* 시안(피그마 Redesign) 실측: 라벨 + 코랄 30% 테두리 컨테이너(안쪽 여백 10,
                  채움 없음) + 그 안에 알약(높이 43, 간격 8).
                  ⚠️ **알약의 면은 시안과 다르다**(2026-09-26 사용자 지시). 시안은 가로
                  그라데이션(`--point-light`→`--point`)에 모드마다 뒤집히는 글자색이었는데,
                  **사용자 질문 말풍선과 같은 면**(평평한 --point + 흰 글자, 두 모드 공통)으로
                  바꿨다 — 제안 질문은 누르면 내 질문이 되는 버튼이라, 내 말풍선과 같은 색이
                  "이건 내가 할 말"이라는 걸 먼저 알려 준다.
                  ⚠️ 면은 `.suggest-pill`(globals.css)이 칠한다. **여기에 Tailwind `bg-point`
                  를 쓰면 안 된다** — 이건 `<button>` 이고 `button.bg-point:not(:disabled)` 가
                  대각선 그라데이션과 글로우를 얹어서, `<div>` 인 말풍선과 오히려 달라진다.
                  그 이유는 globals.css 의 `.suggest-pill` 머리말에 적어 뒀다. */}
              {msg.charged && i === messages.length - 1 && (msg.suggestions?.length ?? 0) > 0 && (
                /* **알약의 좌우 끝을 AI 답변 글자 기둥에 맞춘다**(2026-09-26 사용자 지시).
                   맞춰야 할 선이 둘이다:
                   - 최대 폭 `max-w-[78%]` — 답변 본문과 같은 값이다. 예전엔 `w-full` 이라
                     제안 블록만 대화 영역을 꽉 채워서 답변보다 눈에 띄게 넓었다
                   - 들여쓰기 `px-1` — 알약까지의 거리를 답변의 `px-4`(16)와 같게 만드는 값이다:
                     4(px-1) + 2(테두리) + 10(p-2.5) = 16. `px-4` 로 두면 알약이 28 에서
                     시작해 답변 글자보다 12 안쪽으로 들어간다
                   ⚠️ 시안(피그마 New/Chattingroom_*) 실측은 이 블록이 x 40~372 로 대화 영역을
                   거의 꽉 채우는 모양이었다. **일부러 벗어난 것이다** — 실측이 틀린 게 아니라
                   사용자가 답변 기둥에 맞추기로 정했다. 되돌릴 때 이 문단을 먼저 읽을 것. */
                <div className="mt-3 w-full max-w-[78%] px-1">
