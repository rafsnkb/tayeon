...
//   · 아이콘 단독 금지. 전부 아이콘 + 글자.
//   · 상태를 배지가 아니라 문장으로. "14회" 대신 "이번 달 14회 남았어요".
//   · 최소 글자 크기를 14px로 올리고 본문은 16px.
...
const FIXES = [
  ...
  { what: "이용권 추가", before: "40×40 · 아이콘만", after: "상태 줄의 “이용권 구입” 버튼 48h", why: "44px 미만 + 라벨 없음" },
  { what: "대화방 정보", before: "40×40 · 아이콘만", after: "“더보기” 48h, 글자 포함", why: "〃" },
  { what: "대화 제목", before: "222×24 버튼", after: "제목은 글자, 이름 변경은 더보기 안으로", why: "누를 수 있는지 알 수 없고 24px" },
  { what: "보유 이용권", before: "48×48 아이콘 · 허공에 떠 있음", after: "상단 상태 줄에 문장으로", why: "위치가 떠 있어 정렬이 깨지고 의미가 없다" },
  { what: "“보유 이용권 없음”", before: "12px 글자", after: "16px 문장 + 다음 행동 버튼", why: "빈 상태에 다음 행동이 없었다" },
  { what: "스프레드 선택", before: "칩 → 바텀시트 2단계", after: "현재 선택을 글자로 보여주는 48h 버튼", why: "무엇이 선택돼 있는지 한눈에 안 보였다" },
  { what: "질문 입력", before: "placeholder만", after: "그 위에 보이는 라벨", why: "입력하면 라벨이 사라진다" },
  { what: "보내기", before: "48×48 아이콘만", after: "“보내기” 글자 + 화살표", why: "아이콘만" },
  { what: "궁합 상대 정보", before: "68×14 · 12px", after: "48h 줄 안의 링크, 15px", why: "높이 14px는 정밀 탭을 요구한다" },
  { what: "회사 정보", before: "37×15 · 10px", after: "44h 영역, 13px", why: "10px는 읽히지 않는다" },
];
  ...
  <figure class="frame">
    ...
    <div class="screen" style="${style}">
      ...
      <div class="col">
        ...
        <!-- 상태 줄: 떠 있던 이용권 배지를 여기로. 배지가 아니라 문장이다. -->
        <div class="status">
          <p class="status-text">스탠다드 이용권<br><b>14회</b> 남았어요</p>
          <button class="btn-grad sm">이용권 구입</button>
        </div>
        ...
        <div class="composer">
          <!-- 보이는 라벨. placeholder는 입력을 시작하면 사라진다. -->
          <label class="field-label" for="q-${key}">무엇이 궁금하세요?</label>
          <div class="field" id="q-${key}">이직 시기를 알고 싶어요</div>
          <div class="actions">
            <button class="spread">
              <span class="spread-label">카드 방식</span>
              <span class="spread-value">원 카드&nbsp;<span class="caret">▼</span></span>
            </button>
    ...
    <dl class="facts">
      <div><dt>본문 글자 크기<small>이전 13~14px</small></dt><dd class="note">16px</dd></div>
      <div><dt>가장 작은 글자<small>이전 10px(회사 정보)</small></dt><dd class="note">13px</dd></div>
      <div><dt>가장 작은 터치 영역<small>이전 68×14</small></dt><dd class="pass">48px</dd></div>
...
<style>
  * { box-sizing:border-box; }
  ...
  .screen { position:relative; width:390px; height:860px; overflow:hidden;
  ...
  /* 상단바 — 아이콘 단독 없음. 최소 높이 48. */
  .topbar { position:relative; display:flex; align-items:center; gap:8px; padding:10px 12px;
    background:var(--surface); border-bottom:1px solid var(--line); }
  .bar-btn { display:flex; flex-direction:column; align-items:center; justify-content:center;
    gap:1px; min-width:56px; height:48px; border:0; border-radius:14px; cursor:pointer;
    background:transparent; color:var(--text); font:inherit; font-size:11.5px; font-weight:600;
    transition:background 300ms ease; }
  .bar-btn:hover { background:var(--chip); }
  .bar-btn .ic { font-size:17px; line-height:1; }
  .title { flex:1; text-align:center; font-size:16px; font-weight:700; color:var(--text);
    overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

  /* 상태 줄 — 떠 있던 배지를 없애고 문장으로. */
  .status { position:relative; display:flex; align-items:center; justify-content:space-between;
    gap:12px; padding:12px 16px; background:var(--surface-alt); border-bottom:1px solid var(--line); }
  .status-text { margin:0; font-size:14px; line-height:1.4; color:var(--muted); }
  .status-text b { color:var(--text); font-size:17px; }

  .stream { position:relative; flex:1; overflow:hidden; padding:16px 16px 8px;
    display:flex; flex-direction:column; gap:14px; }
  .msg { max-width:88%; }
  .msg.me { align-self:flex-end; max-width:82%; }
  .who { margin:0 0 4px; font-size:13px; color:var(--muted); }
  .body { margin:0; font-size:16px; line-height:1.65; color:var(--text);
    background:var(--surface); border:1px solid var(--line); border-radius:18px; padding:12px 14px; }
  .msg.me .body { background:var(--grad); color:var(--on-grad); border:0; font-weight:600; }
  .card-name { margin:0 0 6px; font-size:20px; font-weight:700; color:var(--text); }
  .msg .card-name + .body { margin-top:0; }

  .suggest { display:flex; flex-direction:column; gap:8px; }
  .suggest-label { margin:0 0 0 2px; font-size:13px; color:var(--muted); }
  .suggest button { display:flex; align-items:center; gap:8px; text-align:left; font:inherit;
    font-size:15px; font-weight:600; cursor:pointer; border:0; background:var(--tint);
    color:var(--link); border-radius:999px; padding:12px 16px; min-height:48px;
    transition:transform 300ms ease, filter 300ms ease; }
  .suggest button:hover { transform:translateX(3px); filter:brightness(1.05); }
  .suggest .ico { font-size:14px; }

  /* 컴포저 — 보이는 라벨, 48h 컨트롤, 글자 붙은 버튼 */
  .composer { position:relative; padding:12px 16px 4px; background:var(--surface);
    border-top:1px solid var(--line); }
  .field-label { display:block; margin-bottom:6px; font-size:14px; font-weight:600; color:var(--muted); }
  .field { min-height:56px; border-radius:16px; background:var(--page); border:1px solid var(--line);
    padding:15px 14px; font-size:16px; color:var(--text); }
  .actions { display:flex; align-items:stretch; gap:10px; margin-top:10px; }
  .spread { flex:1; display:flex; flex-direction:column; align-items:flex-start; justify-content:center;
    gap:1px; min-height:56px; border:1px solid var(--line); border-radius:16px; cursor:pointer;
    background:var(--page); padding:8px 14px; font:inherit; transition:background 300ms ease; }
  .spread:hover { background:var(--chip); }
  .spread-label { font-size:12px; color:var(--muted); }
  .spread-value { font-size:15px; font-weight:700; color:var(--text); }
  .caret { font-size:11px; color:var(--muted); }
  .btn-grad { border:0; border-radius:16px; cursor:pointer; background:var(--grad);
    color:var(--on-grad); font:inherit; font-weight:700;
    transition:transform 300ms ease, filter 300ms ease; }
  .btn-grad:hover { transform:translateY(-2px); filter:brightness(1.04); }
  .btn-grad.send { min-width:112px; min-height:56px; font-size:16px; display:flex;
    align-items:center; justify-content:center; gap:6px; }
  .btn-grad.sm { min-height:48px; padding:0 18px; font-size:14px; white-space:nowrap; }
  .helper { display:flex; align-items:center; min-height:44px; font-size:15px; color:var(--muted); }
  ...
  .footnote a { font-size:13px; color:var(--muted); text-decoration:underline; }
