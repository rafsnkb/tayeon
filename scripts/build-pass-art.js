 ...
 *
 * 원본(`asset/texture/prizebg_tier_*.jpg`, `timepass_*.png`)은 1254px 정사각으로 합쳐 45MB라
 * 그대로 web 에 올릴 수 없고, **저장소에도 두지 않는다**(.gitignore, 2026-09-25 사용자 결정).
 * 즉 새로 클론한 곳에서는 이 스크립트가 돌지 않는다 — 결과물인 public/pass/*.webp 는 커밋돼
 * 있으므로 앱 빌드에는 지장이 없고, 그림을 바꿀 때만 원본을 받아 와서 다시 돌리면 된다.
